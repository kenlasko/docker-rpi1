...
//document.querySelectorAll('#ExtensionTable tr').forEach(TableRow => {
//	TableRow.addEventListener('click', (event) => {
//		alert(event.target.parentElement.getAttribute("data-src"));
//		const RA = event.target.parentElement.getAttribute("data-src").split(",");
//		LoadinParent(RA[0], RA[1], RA[2], RA[3], RA[4], RA[5], RA[6], RA[7], RA[8], RA[9],
//			RA[10], RA[11], RA[12], RA[13], RA[14], RA[15], RA[16], RA[17], RA[18], RA[19],
//			RA[20], RA[21], RA[22], RA[23], RA[24]);
//	});
//});
...
function VerifyParent() {
	...
	if (!(window.opener && window.opener.open && !window.opener.closed)) {
		window.location = "http://www.skypeoptimizer.com/";
	}
}
